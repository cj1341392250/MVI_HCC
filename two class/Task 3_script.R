...
save(Sample,seed,file = 'model3_sample.Rdata')
#---Established Model------
rm(list = ls())
load('model3_sample.Rdata')
samples <- Sample[[1324]]
set.seed(1324)
if(T){
df <- read.csv('Task 3.csv')
...
fwrite(Combined_df,file = 'combined_df_model3.csv')
...
data.table::fwrite(comparison_results1,file = 'model3_likehold.csv')


#---Train_AUC----
pdf(file = 'Model3_Train_ROC.pdf',width =6,height = 5)
...
legend("bottomright",
       c(paste0("Model AP3 AUC=",round(train_AP_roc$auc,3)), 
         ...
         paste0("Model B3 AUC=",round(train_B1_roc$auc,3)), 
         paste0("Model A3 AUC=",round(train_Clin_roc$auc,3)), 
         paste0("Model C3 AUC=",round(train_C1_roc$auc,3))
       ),
...
dev.off()
#---Test_AUC----
pdf(file = 'Model3_Test_ROC.pdf',width = 6,height = 5)
...
legend("bottomright",
       c(paste0("Model AP3 AUC=",round(test_AP_roc$auc,3)), 
         ...
         paste0("Model B3 AUC=",round(test_B1_roc$auc,3)), 
         paste0("Model A3 AUC=",round(test_Clin_roc$auc,3)), 
         paste0("Model C3 AUC=",round(test_C1_roc$auc,3))
       ),
       col=c("#1C9A35", "#1B4E8C", "#EDB85D", "#6595A3","#555555","#E41A1C"),
       lty=1, lwd=2,bty = "n"
)
...
dca_df$model[dca_df$model=='B1_model']='Model B3' 
dca_df$model[dca_df$model=='A1_model']='Model A3' 
dca_df$model[dca_df$model=='C1_model']='Model C3' 
...
ggsave(filename = 'Model3_DCA.pdf',height = 5,width = 6)
...
pdf(file = 'Model3_calibrate_curve.pdf',width = 5,height = 5)
plot(cal2,
     xlim = c(0,1),
     ylim = c(0,1),
